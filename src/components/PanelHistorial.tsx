...
export default function PanelHistorial() {
  ...
  return (
    <div className="w-full bg-slate-50/50 dark:bg-slate-900/50 p-6 rounded-xl flex flex-col h-full overflow-y-auto custom-scrollbar">
      ...
      <div className="inline-flex gap-1 p-1 mb-6 bg-slate-100 dark:bg-slate-700 rounded-xl w-full sm:w-auto">
        <button
          onClick={() => setVistaActiva("choferes")}
          className={`flex items-center justify-center gap-2 px-5 py-2 rounded-lg font-bold text-sm transition-colors flex-1 sm:flex-initial ${isChofer ? "bg-white dark:bg-slate-800 text-purple-700 dark:text-purple-300 shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-purple-600 dark:hover:text-purple-400"}`}
        >
          <User size={16} /> Choferes
        </button>
        <button
          onClick={() => setVistaActiva("ayudantes")}
          className={`flex items-center justify-center gap-2 px-5 py-2 rounded-lg font-bold text-sm transition-colors flex-1 sm:flex-initial ${!isChofer ? "bg-white dark:bg-slate-800 text-emerald-700 dark:text-emerald-300 shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400"}`}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm flex items-center gap-4 xl:col-span-2">
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-bold text-slate-700 dark:text-slate-200 text-xs">
                      <div className="flex items-center gap-2">
                        {personal.nombre}
                        {esLider && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 text-[10px] font-bold uppercase tracking-wide">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold shrink-0 tabular-nums ${personal.totalViajes === 0 ? "bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300" : index < 5 ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300" : "bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200"}`}
                        >
